...
- **Surface:** the HTTP server is the product. CLI is a client. Effect `HttpApi` (not RPC), OpenAPI, web UI later on the same API + SSE live channel. See ADR 0010.
...
- **Realtime:** In-process bus after sqlite commits + `GET /events` SSE. Sqlite stays source of truth; events are notifications. Not library-wide event sourcing in v1. Web: REST snapshot then EventSource; CLI does not subscribe. See `docs/adrs/0010-daemon-bus-sse.md`.
- **Daemon:** CLI/`npx` ensures a detached server via health probe + spawn if missing; `service start|stop|restart|status`; foreground `serve` for debug. Registration under env-paths state dir. No multi-contender election. See `docs/adrs/0010-daemon-bus-sse.md`.
