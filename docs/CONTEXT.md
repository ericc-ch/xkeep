...
Qwen3-VL-Embedding-2B via official llama.cpp Vulkan (`llama-server`, `--pooling last`, pinned media marker) at **256 image tokens**. Store 2048-d. A drain fiber waits until llama is `ready`, then embeds rows with a null blob. Short GGUF files are deleted and fetched again. Hub GGUFs: PATH `hf` if present (`hf download` into `{cacheDir}/gguf/`); else HTTP. GitHub llama tarball is HTTP only.
