...
- **AppConfig:** `Context.Service` with `make(overrides)`. Two annotated Effect Schemas: file (internal, nested, optional) and resolved (the service contract: listen, dirs, derived paths, llama base URL). HTTP listen comes from the service. See `docs/adrs/0006-appconfig-service.md` and `docs/adrs/0009-config-schemas.md`.
...
- **Config file:** `~/.config/x-bookmarks/config.json` (env-paths config dir), read by the server only. JSON, strict decode (unknown key or bad value fails boot), all keys optional: `listen.host`, `listen.port`, `paths.data`, `paths.cache`, `llama.port`. Never auto-created. Precedence: flags > `X_BOOKMARKS_*` env > file > XDG defaults. CLI reads no file: `--url` > `X_BOOKMARKS_URL` > built-in default. See `docs/adrs/0008-config-file.md`. Resolved config is a second annotated schema; `make` merges then decodes it. Annotations are docs, not runtime. `ConfigError` stays the boot error. Resolved schema is not on `@x-bookmarks/server/api`. See `docs/adrs/0009-config-schemas.md`.
