...
A bookmark has `id` (numeric snowflake), author/handle, text, timestamp, hashtags, urls, avatar, optional quoted tweet, and `media[]` of type `photo` | `video` | `gif`. Videos may include a `poster` URL. Import rejects path-like ids. Export JSON is `{ bookmarks }`. Console snippet: `scripts/dump-bookmarks.js`.
...
- **Intake:** `POST /api/imports` upserts sqlite and returns. No on-disk export copy. Stills download on a daemon fiber. Second import while that fiber runs is `409`. Health `import` is `idle` | `running`. See ADR 0013.
