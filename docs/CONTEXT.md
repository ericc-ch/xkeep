...
- **Surface:** one bin `x-bookmarks`. `service` is process lifetime (`start|stop|restart|status|serve`). `api` is a generated OpenAPI port (`HttpApi.reflect`); stdout JSON; no spawn. Bare command: ensure + print url (no browser until real UI). Effect `HttpApi` (not RPC), OpenAPI, web UI later on the same API + SSE. Loopback, no auth. See ADR 0010.
...
- **Packages:** `@x-bookmarks/server` (embed, HTTP, dump schema, `HttpApi`) and `@x-bookmarks/cli` (user-facing `x-bookmarks` bin). See `docs/adrs/0004-nub-workspaces.md`, `0005`, `0010`.
