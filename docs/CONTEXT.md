...
Local X bookmarks app. Intake is a JSON dump (`x-bookmarks-dump/1`). No paid X API. No Chrome extension in v1. "Library" is not a product noun — the pile is bookmarks; the process is the daemon.
...
| daemon | Long-lived `x-bookmarks` server process on loopback. |
...
- **Surface:** one bin `x-bookmarks` (subcommands). HTTP server is the product; CLI verbs are clients with ensure. Effect `HttpApi` (not RPC), OpenAPI, web UI later on the same API + SSE. Loopback, no auth. Bare command: ensure + print url (no browser until real UI). See ADR 0010.
...
- **Packages:** `@x-bookmarks/server` (embed, HTTP, dump schema, `HttpApi`) and `@x-bookmarks/cli` (HTTP client). User-facing: one bin `x-bookmarks`. See `docs/adrs/0004-nub-workspaces.md`, `0005`, `0010`.
...
- **Daemon:** Ensure detached server via health + spawn; `service start|stop|restart|status`; foreground `serve`. `service.json` under state dir (no password). No multi-contender election. See ADR 0010.
