...
- **Packages:** `@xkeep/server` (`layer` + `./schema` dump + `./http` wire) and `@xkeep/cli` (user-facing `xkeep` bin, owns `service.json`). See `docs/adrs/0004-nub-workspaces.md`, `0005`, `0010`, `0012`.
...
- **Library UI:** Spatial map of the pile at `/`. Search + filters highlight ids only. Search hits are id, score, handle, author, text — no disk `stillPaths`. `GET /api/bookmarks` one shot (includes cached UMAP `x`,`y` when present); `GET /api/bookmarks/:id` for the rest; `GET /api/media/:name` (basename only, inside `mediaDir`). Rows without coords sit on a client grid. `GET /api/clusters?k=` (default 12). No `minSize`. Tag ids are UUIDs. Thumb is the first still. `packages/web` not built yet. See ADR 0014.
- **Ship order:** (1) daemon/ensure (done) (2) bus+sse + tag + cluster + pile + media APIs (done) (3) real UI at `/`.
