...
- Config file schema: nested, all keys optional, disk JSON only. Shape gate: `Unknown` leaves, strict `onExcessProperty: "error"`. Internal to the server config module. Not exported through `@xkeep/server/api`.
...
The resolved schema is used inside `packages/server`. It is not added to `@xkeep/server/api` until another package needs it.
