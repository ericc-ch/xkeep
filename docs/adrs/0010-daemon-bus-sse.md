...
- Keep Effect `HttpApi` as the contract. Reads and mutations stay REST (GET/POST/PUT/PATCH) and appear in OpenAPI. Mount the API under `/api` so `/` stays free for the canvas UI (`/api/health`, `/api/openapi.json`, `/api/docs`).
